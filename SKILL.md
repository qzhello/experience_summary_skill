...
0. **Restate First & 类别判定**(在任何文件操作之前一次问完):
   用**自然口语**复述给用户,**不要展示 `<...>` 这种填空模板**。要传达的信息:
   - 你怎么理解这个任务(项目名 + 你看出来这是什么类型的项目)
   - 判定理由(从 README / 构建文件 / 目录结构看到的信号,一句话)
   - 类别不确定时,列 2-3 个候选
   - 接下来你会做什么(拷模板 + 建目录 + 用元数据填快照,业务一律留"待补")
   - 让用户回 yes / 改类别 / 选候选

   **示例输出**(填好之后给用户的样子):

   ```
   看着像 experience-summary 这个 Python 工具项目 —— scripts/ 下都是 stdlib-only
   小工具,根目录是规约 + 模板。我按 tool 类来建经验目录可以吗?

   不太确定的话,也可以走 application(兜底类)。

   接下来我会:从 templates/ 拷 4 个文件 + 建 2 个空目录,只用 README、scripts
   入口、目录结构这些来推断"项目快照"块,业务细节都先留"待补"等你来填。

   确认就 yes;要改类别告诉我;要不建直接说停。
   ```
...
3. **填充硬边界(防幻觉)**:初始化只能从这些来源推断 AGENT.md 的"项目快照":
   ...
   **禁止**:扫描业务代码后编造"模块职责"、推测"设计意图"、复述未读源码的行为。
   不确定的字段写 `<待补>` 或 `<待补 — 例:...>`(模板已是这种格式),给用户友好的填写提示,不用裸 `<TBD>`。
...
10. **不暴露内部代号**:对用户回复**用自然语言**,不甩 `G1 / G4 / path A-E / E001 / W006 / SoT / status=stale` 这类代号。这些是 AI 内部一致性用的标签,用户不需要看到。
    - ❌ "per G4 批量确认,我列出 6 个字段"
    - ✅ "我一次性把要确认的 6 个字段列在下面,看一眼有没有要改的"
    - ❌ "path E 收尾必须输出 Done Contract"
    - ✅ "review 结束我会汇报这次具体做了什么、漏了什么、建议下一步"
